            ...
            <div className="bg-white p-3 rounded-lg">
                {
                    allVoucher && allVoucher.length > 0 ?
                        allVoucher.map((item, index) => {
                            const isExpired = compareDay(format(today, 'dd/MM/yyyy'), format(item.expireDate, "dd/MM/yyy"));
                            return (
                                <div key={index} className='flex justify-between items-center w-full relative mb-3'>
                                    <div className='flex flex-row gap-2 justify-center'>
                                        ...
                                        <div className={`flex flex-col ${isExpired && item.status === "NotUsed" ? "" : "text-gray-400"}`}>
                                            <h2 className='font-bold text-lg'>{item.voucherCode}</h2>
                                            <h4 className='text-sm'>{item.description}</h4>
                                            <h5 className='text-sm'>Hạn sử dụng: {format(item.expireDate, "dd/MM/yyy")}</h5>
                                        </div>
                                    </div>
                                    {
                                        isExpired && item.status === "NotUsed" ?
                                            <p className={`rounded-md p-2 px-4 font-bold text-white bg-main`}>Chưa sử dụng</p>
                                            :
                                            <p className='rounded-md p-2 px-4 font-bold text-gray-500 bg-gray-200'>Đã hết</p>
                                    }
                                </div>
                            )
                        })
