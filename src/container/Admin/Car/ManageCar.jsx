...
function ManageCar() {
    ...
    const handleConfirmCar = async (carId) => {
        try {
            dispatch(setShowLoading())
            let res = await confimCarByAdmin(carId, token)
            if (res) {
                                    ...
                                    <div className="flex justify-between mt-3">
                                        {
                                            car.status === "Approving" ?
                                                <button className="w-[100%] hover:opacity-80 p-1 px-2 text-sm bg-black text-white font-semibold rounded-md" onClick={() => handleConfirmCar(car.carId)}>Xác thực</button>
                                                :
                                                <p className="w-full p-1 text-center text-sm text-white bg-main font-semibold rounded-md cursor-default">Đã xác thực <i className="fa-solid fa-check"></i></p>
                                        }
                                    </div>
                                    <div className="flex justify-between mt-3">
                                        <button className="w-[30%] hover:opacity-80 p-1 px-2 text-sm bg-main text-white font-semibold rounded-md" onClick={() => handleOpenModalView(car.carId)}>Xem</button>
                                        <button className="w-[30%] hover:opacity-80 p-1 px-2 text-sm bg-main text-white font-semibold rounded-md" onClick={() => handleOpenModalEdit(car.carId)}>Sửa</button>
                                        <button className="w-[30%] hover:opacity-80 p-1 px-2 text-sm bg-main text-white font-semibold rounded-md" onClick={() => handleOpenModalDelete(car.carId)}>Xóa</button>
                                    </div>
                                </div>
                            )
                        })
